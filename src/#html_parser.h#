...
#define html_parser_append(parser, c)					\
	...
	if((parser)->stack_size && (parser)->on_node_content &&		\
	   !(parser)->on_node_content((parser)->stack[(parser)->stack_size-1].elem, \
				      (parser)->buffer[HTML_PARSER_BUFFER], (parser)->buffer_size[HTML_PARSER_BUFFER], (parser)->arg)) { \
	    HTML_PARSER_LOG("Failure beacause 'on_node_content' returned false"); \
