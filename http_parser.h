...
// TODO: Add something like response_code
// TODO: Do something about content_length. Do not recompute
// the size every time
...
static const char *http_parser_string_cstr_capital_content_length = "Content-Length";
static const char *http_parser_string_cstr_content_length = "content-length";
static const char *http_parser_string_cstr_capital_transfer_encoding = "Transfer-Encoding";
static const char *http_parser_string_cstr_transfer_encoding = "transfer-encoding";
static const char *http_parser_string_cstr_capital_chunked = "Chunked";
  ...
  while(s.len) {
    ...
    if(parser->state == HTTP_PARSER_STATE_IN_HEADER_CONTENT_LENGTH) {
      if( (http_parser_string_eq(key, http_parser_string_cstr_transfer_encoding) ||
	   http_parser_string_eq(key, http_parser_string_cstr_capital_transfer_encoding)) &&
	  (http_parser_string_eq(value, http_parser_string_cstr_chunked) ||
	   http_parser_string_eq(value, http_parser_string_cstr_capital_chunked))) {
	// invalid state. content length + encoding chunked
	return HTTP_PARSER_RET_ABORT;
      }      
    } else {
      if(http_parser_string_eq(key, http_parser_string_cstr_content_length) ||
	 http_parser_string_eq(key, http_parser_string_cstr_capital_content_length) ) {	
	// invalid state. content length + encoding chunked
	return HTTP_PARSER_RET_ABORT;
      }
    }
...
HTTP_PARSER_DEF Http_Parser_Ret http_parser_consume(Http_Parser *parser, const char *data, size_t size) {
  ...
  case HTTP_PARSER_STATE_CHUNKED: {
    ...
    if(i < size)
      return http_parser_consume(parser, data + i, size - i);
  ...
  case HTTP_PARSER_STATE_CHUNKED_CONSUME: {
    ...
    if(i==0)
      return http_parser_consume(parser, data + 2, size - 2);
    ...
    if(i + off < size) {
      ...
      return http_parser_consume(parser, data + i + off, size - i - off);
    }
