...
//TODO: maybe cleanup functions (win32_tsl_socket_write, ...)
//TODO: get rid of assertions
//TODO: close win32_tsl_socket
...
HTTP_DEF bool http_socket_write_plain(const char *data, size_t size, Http *http);
HTTP_DEF bool http_socket_read_plain(char *data, size_t size, Http *http, size_t *read);
...
HTTP_DEF bool http_socket_connect_tls(Http *http, const char *hostname) {
  ...
  /*
  // connect to server
  if (!WSAConnectByNameA(s->sock, hostname, port_cstr, NULL, NULL, NULL, NULL, NULL, NULL)) {
    return false;
  }
  */
...
HTTP_DEF bool http_socket_write(const char *data, size_t size, void *_http) {
  ...
  // If the write fails, because of any error, but we should continue trying to write.
   ...
   while (size != 0) {
        ...
        assert(http->win32_socket.sizes.cbHeader + http->win32_socket.sizes.cbMaximumMessage + http->win32_socket.sizes.cbTrailer <= sizeof(wbuffer));
...
HTTP_DEF bool http_socket_read_plain(char *buffer, size_t buffer_size, Http *http, size_t *read) {
...
HTTP_DEF bool http_socket_read(char *buffer, size_t buffer_size, void *_http, size_t *read) {
  ...
  while (buffer_size != 0) {
	...
	{
	  SecBuffer buffers[4];
	  assert(http->win32_socket.sizes.cBuffers == ARRAYSIZE(buffers));
