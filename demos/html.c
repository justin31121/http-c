...
Tag *query(Tag *root) {
  ...
  static size_t needle_len = sizeof(needle) - 1;
  
  Content *c = root->content;
  while(c) {       
    if(c->text != NULL) {
      if(c->text_len >= needle_len && strncmp(c->text, needle, needle_len) == 0) {
	return root;
      }
    } else {
      Tag *tag = query(c->tag);
...
int main() {
  ...
  for(i=0;i<buffer_size;i++) {
    ...
    if(len > 100) len = 100;
    if(ret == HTML_PARSER_RET_ABORT) {
      fprintf(stderr, "ERROR: Failed to parse at [%zu/%zu]: '%.*s'\n", i, buffer_size, len, buffer + i);
      return 1;
    } else if(ret == HTML_PARSER_RET_CONTINUE) {
      //pass
    } else if(i != buffer_size - 1) {
      fprintf(stderr, "ERROR: Parsed to early at [%zu/%zu]: '%.*s'\n", i, buffer_size, len, buffer + i);
      return 1;
    } else {

      Tag *tag = query(root);
      const char *file_path = "rsc\\initialData.json";
